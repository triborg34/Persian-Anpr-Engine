    ...
    def __init__(self, plate_model_path, char_model_path, arvand_model_path):
        ...
        try:
            self.model_plate = torch.hub.load(
                'yolov5', 'custom', plate_model_path, source='local', device=device, force_reload=False)
        ...
        try:
            self.model_char = torch.hub.load(
                'yolov5', 'custom', char_model_path, source='local', force_reload=False)
            self.model_char.eval()
        except ModuleNotFoundError as e:
            logger.error(f"Error loading character model: {e}")
            raise

        try:
            self.carmodel = YOLO('model/yolo11n.pt', verbose=False).to(device)
        except Exception as e:
            logger.error(f"Error loading car model: {e}")
            raise
...
def clear_memory():
    logger.warning("Clearing GPU memory cache...")
    # Move models to CPU temporarily
    for model_attr in ['model_plate', 'model_char', 'carmodel']:
        if hasattr(models, model_attr):
            model = getattr(models, model_attr)
            model.to('cpu')
    # Clear CUDA cache
    torch.cuda.empty_cache()
    gc.collect()
    # Move models back to GPU if needed
    for model_attr in ['model_plate', 'model_char', 'carmodel']:
        if hasattr(models, model_attr):
            model = getattr(models, model_attr)
            model.to(device)

# Improved GPU memory management as a context manager
@contextmanager
def clear_memory_on_oom():
    try:
        yield
    except RuntimeError as e:
        if 'CUDA out of memory' in str(e):
            logger.warning("CUDA OOM error. Clearing GPU memory cache...")
            torch.cuda.empty_cache()
            gc.collect()
        else:
            raise
...
# Improved Frame Producer with exponential backoff
MAX_RETRIES = 5
...
async def transmit_frames(websocket, path):
    ...
    try:
        while not stop_event.is_set():
            ...
            if not buffer.empty():
                frame = buffer.get()
                with clear_memory_on_oom():
                    with torch.no_grad():
                        if torch.cuda.memory_allocated(device) > 1.5e9:
